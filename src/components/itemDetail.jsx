...
export default function ItemDetail() {
  ...
  const handleAddCart = () => {
    const alreadyInCart = allCarts.find(
      (cart) => cart.id === myDatas[index].id
    );
    if (alreadyInCart) {
      return;
    }
    const newCart = {
      name: myDatas[index].name,
      price: myDatas[index].value,
      id: myDatas[index].id,
      quantity: 1,
      initialPrice: myDatas[index].value,
      icon: myDatas[index].source,
    };
